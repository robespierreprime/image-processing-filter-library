...
class DitherFilter(BaseFilter):
    ...
    Parameters:
        ...
        bayer_size (int): Size of Bayer matrix for Bayer dithering (2, 4, or 8)
    ...
    def __init__(self, pattern_type: str = "floyd_steinberg", levels: int = 8, 
        ...
        Args:
            ...
            bayer_size: Size of Bayer matrix for Bayer dithering (2, 4, or 8, default 4)
        """
        super().__init__(
            name="dither",
    ...
    def _generate_bayer_matrix(self, size: int) -> np.ndarray:
        ...
        Args:
            size: Size of the matrix (2, 4, or 8)
        ...
        if size == 2:
            ...
            # 4x4 Bayer matrix built from 2x2
            base = self._generate_bayer_matrix(2)
            return np.array([
                [4*base[0,0], 4*base[0,1], 4*base[0,0]+2, 4*base[0,1]+2],
                [4*base[1,0], 4*base[1,1], 4*base[1,0]+2, 4*base[1,1]+2],
                [4*base[0,0]+3, 4*base[0,1]+3, 4*base[0,0]+1, 4*base[0,1]+1],
                [4*base[1,0]+3, 4*base[1,1]+3, 4*base[1,0]+1, 4*base[1,1]+1]
            ], dtype=np.float64)
        
        elif size == 8:
            # 8x8 Bayer matrix built from 4x4
            base = self._generate_bayer_matrix(4)
            top_left = 4 * base
            top_right = 4 * base + 2
